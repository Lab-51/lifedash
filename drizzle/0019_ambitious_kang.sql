...
ALTER TABLE "projects" ADD COLUMN IF NOT EXISTS "hourly_rate" real;--> statement-breakpoint
ALTER TABLE "focus_sessions" ADD COLUMN IF NOT EXISTS "billable" boolean DEFAULT true NOT NULL;--> statement-breakpoint
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'project_agent_messages_project_id_projects_id_fk'
  ) THEN
