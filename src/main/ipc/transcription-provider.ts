...
import { transcriptionProviderTypeSchema, transcriptionApiKeyProviderSchema } from '../../shared/validation/schemas';
...
export function registerTranscriptionProviderHandlers(): void {
  ...
  ipcMain.handle('transcription:set-provider', async (_event, type: unknown) => {
  ...
  ipcMain.handle('transcription:set-api-key', async (_event, provider: unknown, apiKey: unknown) => {
  ...
  ipcMain.handle('transcription:test-provider', async (_event, type: unknown) => {
    const validType = validateInput(transcriptionProviderTypeSchema, type);
