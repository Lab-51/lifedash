 ...
 */
function logCardActivity(cardId: string, action: string, details?: Record<string, unknown>): void {
  const db = getDb();
...
export function registerCardHandlers(): void {
  ...
  ipcMain.handle('cards:list-by-board', async (_event, boardId: unknown) => {
    ...
    const boardColumns = await db.select().from(columns).where(eq(columns.boardId, validBoardId));
    ...
    const allCardLabelRows = await db.select().from(cardLabels).where(inArray(cardLabels.cardId, cardIds));

    // Query 4: Batch-fetch all labels referenced by these cards
    const labelIds = [...new Set(allCardLabelRows.map((cl) => cl.labelId))];
    const allLabels = labelIds.length > 0 ? await db.select().from(labels).where(inArray(labels.id, labelIds)) : [];

    // Build card -> labels lookup via shared utility
    const cardLabelMap = buildCardLabelMap(allCardLabelRows, allLabels as unknown as Label[]);
  ...
  ipcMain.handle('cards:create', async (_event, data: unknown) => {
  ...
  ipcMain.handle('cards:update', async (_event, id: unknown, data: unknown) => {
    ...
    if (input.recurrenceEndDate !== undefined) {
      setData.recurrenceEndDate = input.recurrenceEndDate ? new Date(input.recurrenceEndDate) : null;
    }
    const [card] = await db.update(cards).set(setData).where(eq(cards.id, validId)).returning();

    // Check if completed changed from false -> true AND card has recurrenceType
    let spawnedCard = null;
    if (currentCard && !currentCard.completed && card.completed && card.recurrenceType) {
  ...
  ipcMain.handle('cards:move', async (_event, id: unknown, columnId: unknown, position: unknown) => {
    ...
    const siblingsInTarget = await db
      .select()
      .from(cards)
      .where(and(eq(cards.columnId, moveData.columnId), eq(cards.archived, false)))
      .orderBy(asc(cards.position));
    ...
    await db.transaction(async (tx) => {
      for (const upd of updates) {
        if (upd.id === validId) {
          ...
          await tx.update(cards).set({ position: upd.position }).where(eq(cards.id, upd.id));
        }
      }
    });
  ...
  ipcMain.handle('labels:list', async (_event, projectId: unknown) => {
    ...
    const db = getDb();
    return db.select().from(labels).where(eq(labels.projectId, validProjectId)).orderBy(asc(labels.name));
  });

  ipcMain.handle('labels:create', async (_event, data: unknown) => {
  ...
  ipcMain.handle('labels:update', async (_event, id: unknown, data: unknown) => {
    const validId = validateInput(idParamSchema, id);
    const input = validateInput(updateLabelInputSchema, data);
    const db = getDb();
    const [label] = await db.update(labels).set(input).where(eq(labels.id, validId)).returning();
    return label;
  });
  ...
  ipcMain.handle('labels:attach', async (_event, cardId: unknown, labelId: unknown) => {
    const validCardId = validateInput(idParamSchema, cardId);
    const validLabelId = validateInput(idParamSchema, labelId);
    const db = getDb();
    await db.insert(cardLabels).values({ cardId: validCardId, labelId: validLabelId }).onConflictDoNothing();
  });

  ipcMain.handle('labels:detach', async (_event, cardId: unknown, labelId: unknown) => {
    const validCardId = validateInput(idParamSchema, cardId);
    const validLabelId = validateInput(idParamSchema, labelId);
    const db = getDb();
    await db.delete(cardLabels).where(and(eq(cardLabels.cardId, validCardId), eq(cardLabels.labelId, validLabelId)));
  });
  ...
  ipcMain.handle('card:addComment', async (_event, input: unknown) => {
  ...
  ipcMain.handle('card:updateComment', async (_event, id: unknown, content: unknown) => {
    const validId = validateInput(idParamSchema, id);
  ...
  ipcMain.handle('card:getRelationships', async (_event, cardId: unknown) => {
    const validCardId = validateInput(idParamSchema, cardId);
    const db = getDb();
    // Get relationships where this card is source or target
    const asSource = await db.select().from(cardRelationships).where(eq(cardRelationships.sourceCardId, validCardId));
    const asTarget = await db.select().from(cardRelationships).where(eq(cardRelationships.targetCardId, validCardId));
    ...
    const cardTitleRows =
      allCardIds.length > 0
        ? await db.select({ id: cards.id, title: cards.title }).from(cards).where(inArray(cards.id, allCardIds))
        : [];
  ...
  ipcMain.handle('card:addRelationship', async (_event, input: unknown) => {
  ...
  ipcMain.handle('card:deleteRelationship', async (_event, id: unknown) => {
    ...
    const [rel] = await db.select().from(cardRelationships).where(eq(cardRelationships.id, validId));
  ...
  ipcMain.handle('cards:getRelationshipsByBoard', async (_event, boardId: unknown) => {
    ...
    const boardColumns = await db.select({ id: columns.id }).from(columns).where(eq(columns.boardId, validBoardId));
    const columnIds = boardColumns.map((c) => c.id);
    if (columnIds.length === 0) return [];
    ...
    const asSource = await db.select().from(cardRelationships).where(inArray(cardRelationships.sourceCardId, cardIds));
    const asTarget = await db.select().from(cardRelationships).where(inArray(cardRelationships.targetCardId, cardIds));
  ...
  ipcMain.handle('card:addChecklistItemsBatch', async (_event, input: unknown) => {
    ...
    const items = await db.insert(cardChecklistItems).values(values).returning();
    return items;
  });
  ...
  ipcMain.handle('card:generate-description', async (_event, cardId: unknown) => {
    ...
    if (column) {
      ...
      if (board) {
        const [proj] = await db.select({ name: projects.name }).from(projects).where(eq(projects.id, board.projectId));
        project = proj;
      }
    }

    // Get card labels for additional context
    const cardLabelRows = await db.select().from(cardLabels).where(eq(cardLabels.cardId, validCardId));
    const labelIds = cardLabelRows.map((cl) => cl.labelId);
    const labelRows = labelIds.length > 0 ? await db.select().from(labels).where(inArray(labels.id, labelIds)) : [];
...
Format as a single HTML paragraph (<p> tag).`;
  ...
  ipcMain.handle('card-templates:list', async (_event, projectId?: unknown) => {
    ...
    const where = projectId
      ? or(eq(cardTemplates.projectId, validateInput(idParamSchema, projectId)), isNull(cardTemplates.projectId))
  ...
  ipcMain.handle('card-templates:save-from-card', async (_event, cardId: unknown, name?: unknown) => {
    const validCardId = validateInput(idParamSchema, cardId);
    const db = getDb();
