...
export const cardAgentThreads = pgTable('card_agent_threads', {
  ...
  index('card_agent_threads_card_id_idx').on(table.cardId),
]);
...
export const cardAgentMessages = pgTable('card_agent_messages', {
  ...
  threadId: uuid('thread_id')
    .references(() => cardAgentThreads.id, { onDelete: 'cascade' }),
