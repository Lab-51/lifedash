...
export const projectAgentThreads = pgTable(
  ...
  (table) => [index('project_agent_threads_project_id_idx').on(table.projectId)],
);

export const projectAgentMessages = pgTable(
  'project_agent_messages',
  {
    ...
    projectId: uuid('project_id')
      .notNull()
      .references(() => projects.id, { onDelete: 'cascade' }),
    threadId: uuid('thread_id').references(() => projectAgentThreads.id, { onDelete: 'cascade' }),
