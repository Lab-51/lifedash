...
const inputCls =
  'px-2 py-1.5 text-sm rounded-lg border border-[var(--color-border)] bg-surface-950 text-[var(--color-text-primary)] dark:[color-scheme:dark] focus:outline-none focus:border-[var(--color-accent-dim)]';
...
export default function FocusPage() {
  ...
  const handleExport = useCallback(() => {
    ...
    exportCSV(
      ...
      startDate,
      endDate,
      projectId ? activeProjects.find((p) => p.id === projectId)?.name : undefined,
    );
  ...
  const controls = (
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex rounded-lg border border-[var(--color-border)] overflow-hidden">
        {PERIODS.map(({ key, label }) => (
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium transition-colors ${period === key ? 'bg-[var(--color-accent-muted)] text-[var(--color-accent)] border-[var(--color-border-accent)]' : 'bg-[var(--color-chrome)] text-[var(--color-text-secondary)] hover:bg-[var(--color-accent-subtle)]'}`}
          >
            {label}
          </button>
        ))}
      ...
      <HudSelect
        ...
        compact
        options={[{ value: '', label: 'All Projects' }, ...activeProjects.map((p) => ({ value: p.id, label: p.name }))]}
      />
      <HudSelect
      ...
      <button
        ...
        className="ml-auto btn-primary clip-corner-cut-sm inline-flex items-center gap-1.5 px-4 py-1.5 text-sm font-medium"
      >
      ...
      {loading ? (
        <div className="space-y-4">
          <div className="grid grid-cols-4 gap-4">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="h-20 bg-surface-100 dark:bg-surface-800 rounded-2xl animate-pulse" />
            ))}
          </div>
          ...
          <div className={`grid gap-4 ${summary.billableCost > 0 ? 'grid-cols-5' : 'grid-cols-4'}`}>
            <div className={`${cardCls} p-4`}>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="font-hud text-[0.625rem] text-[var(--color-text-muted)]">Total Sessions</span>
              </div>
            ...
            <div className={`${cardCls} p-4`}>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="font-hud text-[0.625rem] text-[var(--color-text-muted)]">Total Time</span>
              </div>
              <p className="font-data text-2xl font-bold text-[var(--color-text-primary)]">
                {fmt(summary.totalMinutes)}
              </p>
              {billableFilter === '' &&
                summary.billableMinutes > 0 &&
                summary.billableMinutes < summary.totalMinutes && (
            ...
            <div className={`${cardCls} p-4`}>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="font-hud text-[0.625rem] text-[var(--color-text-muted)]">Avg Session</span>
              </div>
              <p className="font-data text-2xl font-bold text-[var(--color-text-primary)]">
                {summary.avgSessionMinutes}m
              </p>
            </div>
            <div className={`${cardCls} p-4`}>
              <div className="flex items-center gap-1.5 mb-1">
                <Calendar size={16} className="text-[var(--color-accent)]" />
                <span className="font-hud text-[0.625rem] text-[var(--color-text-muted)]">Active Days</span>
              </div>
              <p className="font-data text-2xl font-bold text-[var(--color-text-primary)]">
                {summary.activeDays} of {totalDays}
              </p>
            </div>
            {summary.billableCost > 0 && (
              <div className={`${cardCls} p-4`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="font-hud text-[0.625rem] text-[var(--color-text-muted)]">Billable Amount</span>
                </div>
                <p className="font-data text-2xl font-bold text-[var(--color-text-primary)]">
                  {fmtCost(summary.billableCost)}
                </p>
              </div>
            )}
          </div>
                  ...
                  return (
                    <div key={pb.projectId ?? '_none'} className="flex items-center gap-3">
                      <span
                        ...
                        style={{ backgroundColor: pb.projectColor || '#94a3b8' }}
                      />
                      <span className="text-sm text-surface-900 dark:text-surface-100 w-36 truncate shrink-0">
                        {pb.projectName || 'No project'}
                      </span>
                      <div className="flex-1 h-2.5 bg-surface-100 dark:bg-surface-800 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full transition-all duration-300"
                          style={{ width: `${Math.max(pct, 1)}%`, backgroundColor: pb.projectColor || '#94a3b8' }}
                        />
                      </div>
                      <span className="text-sm font-medium text-surface-900 dark:text-surface-100 w-20 text-right shrink-0">
                        {fmt(pb.minutes)}
                      </span>
                      <span className="text-xs text-surface-500 w-20 text-right shrink-0">
                        {pb.sessions} session{pb.sessions !== 1 ? 's' : ''}
                      </span>
                      {pb.cost !== null && pb.cost > 0 && (
                        <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 w-20 text-right shrink-0">
                          {fmtCost(pb.cost)}
                        </span>
                      )}
                    </div>
            ...
            <div className={`${cardCls} p-5`}>
                ...
                {chartData.map((b) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        height: `${h}%`,
                        backgroundColor: b.minutes > 0 ? 'rgb(16 185 129)' : 'var(--color-surface-300)',
                      }}
                      title={`${b.label}: ${fmt(b.minutes)} (${b.sessions} session${b.sessions !== 1 ? 's' : ''})`}
                    />
                  );
                })}
              </div>
                ...
                {chartData.length <= 7
                  ? chartData.map((b) => (
                      <div key={b.key} className="flex-1 text-center">
                        <span className="text-[0.5rem] text-surface-500">
                          {chartGranularity === 'daily'
                            ? new Date(b.key).toLocaleDateString('en-US', { weekday: 'short' })
                            : b.label}
                        </span>
                      </div>
                    ))
                  : chartData.map((b, i) => {
                      ...
                      const lbl =
                        chartGranularity === 'monthly'
                          ? new Date(b.key + '-01').toLocaleDateString('en-US', { month: 'short' })
                          : String(new Date(b.key).getDate());
                      return (
                        <div key={b.key} className="flex-1 text-center">
                          {i % iv === 0 ? <span className="text-[0.5rem] text-surface-500">{lbl}</span> : null}
                        </div>
                      );
                    })}
              </div>
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-surface-200 dark:border-surface-700">
                <span className="text-xs text-surface-500 font-medium">
                  {chartGranularity === 'daily' && chartData.length === 7
                    ? 'Weekly Activity'
                    : chartGranularity === 'monthly'
                      ? 'Monthly Activity'
                      : chartGranularity === 'weekly'
                        ? 'Weekly Activity'
                        : `${chartData.length}-Day Activity`}
                </span>
          ...
          <div className={`${cardCls} p-5`}>
                          ...
                          {editingId === s.id ? (
                            ...
                            <div className="px-3 py-3 rounded-lg bg-surface-50 dark:bg-surface-800/50 space-y-2">
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={cancelEdit}
                                  className="p-1.5 rounded-md bg-surface-200 dark:bg-surface-700 hover:bg-surface-300 dark:hover:bg-surface-600 text-surface-700 dark:text-surface-300 transition-colors"
                                  title="Cancel"
                                >
                                  <X size={14} />
                                </button>
                            ...
                            <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-surface-800/50 transition-colors">
                              ...
                              <span className="text-xs text-surface-400 w-28 truncate shrink-0">
                                {s.projectName || 'No project'}
                              </span>
                              <span className="text-xs text-surface-600 dark:text-surface-300 truncate flex-1 min-w-0">
                                {s.cardTitle || ''}
                              </span>
                              {s.note && (
                                <span className="text-xs text-surface-400 truncate max-w-[160px]">{s.note}</span>
                              )}
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                <button
                                  onClick={() => startEdit(s)}
                                  className="p-1 rounded hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-400 hover:text-surface-600 dark:hover:text-surface-200 transition-colors"
                                  title="Edit session"
                                >
                                  <Pencil size={13} />
                                </button>
                                <button
                                  ...
                                  className="p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-surface-400 hover:text-red-500 transition-colors"
            ...
            {allSessions.length > displayCount && (
              <div className="text-center pt-3">
                <button
                  onClick={() => setDisplayCount((c) => c + PAGE)}
