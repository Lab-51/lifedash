            ...
            {showReward ? (
              ...
              <div className="text-center space-y-4 py-4">
                ...
                {rewardStats && rewardStats.currentStreak > 0 && (
                  <p className="text-amber-400 font-semibold">{rewardStats.currentStreak} day streak!</p>
                )}
              ...
              <>
                ...
                <div className="text-center py-2">
                  ...
                  <p className="text-surface-400 text-sm mt-1">
                    {focusedCardTitle ? (
                      <>
                        on &ldquo;<span className="text-surface-700 dark:text-surface-300">{focusedCardTitle}</span>
                        &rdquo;
                      </>
                    ) : (
                      'General focus'
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2.5 cursor-pointer">
                    ...
                    >
                      <span
                        className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${billable ? 'translate-x-4' : ''}`}
                      />
                    </button>
                  ...
                  {hourlyRate != null && billable && (
                    <span className="text-xs text-emerald-600 dark:text-emerald-400">
                      ~${(billableHours(actualDuration) * hourlyRate).toFixed(2)} ({actualDuration}m →{' '}
                      {billableHours(actualDuration)}h @ ${hourlyRate}/hr)
                    </span>
                  )}
                </div>

                {/* Session count */}
                <p className="text-xs text-surface-500 text-center">Session #{sessionCount} today</p>
