...
const PRIORITY_OPTIONS: { value: CardPriority; label: string; activeClass: string; inactiveClass: string }[] = [
  {
    value: 'low',
    label: 'LOW',
    activeClass: 'bg-emerald-500/30 text-emerald-400 ring-1 ring-emerald-500/50',
    inactiveClass: 'text-surface-400 hover:text-emerald-400',
  },
  {
    value: 'medium',
    label: 'MED',
    activeClass: 'bg-blue-500/30 text-blue-400 ring-1 ring-blue-500/50',
    inactiveClass: 'text-surface-400 hover:text-blue-400',
  },
  {
    value: 'high',
    label: 'HIGH',
    activeClass: 'bg-amber-500/30 text-amber-400 ring-1 ring-amber-500/50',
    inactiveClass: 'text-surface-400 hover:text-amber-400',
  },
  {
    value: 'urgent',
    label: 'URG',
    activeClass: 'bg-red-500/30 text-red-400 ring-1 ring-red-500/50',
    inactiveClass: 'text-surface-400 hover:text-red-400',
  },
];
...
function CardDetailModal({ card, onUpdate, onClose }: CardDetailModalProps) {
  ...
  const editor = useEditor({
    extensions: [StarterKit, Placeholder.configure({ placeholder: 'Add a description...' })],
  ...
  // Label handlers
  const unattachedLabels = labels.filter((l) => !card.labels?.some((cl) => cl.id === l.id));
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-[2px]">
        ...
        >
          <div
            className={`hud-panel-accent clip-corner-cut shadow-2xl w-full flex flex-col overflow-hidden transition-all duration-300 ease-out ${
              showAgent ? 'max-w-[95vw] xl:max-w-[1400px]' : 'max-w-5xl'
            } max-h-[90vh] mx-4`}
          >
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden">
              <div className="flex-1 overflow-y-auto min-w-0 flex flex-col md:flex-row">
                                ...
                                <>
                                  ...
                                  {dbTemplates.map((template) => (
                                    <button
                                      ...
                                      className="group/tpl flex items-center gap-2 w-full px-3 py-1.5 text-sm text-[var(--color-text-secondary)] hover:bg-surface-100 dark:hover:bg-surface-800 hover:text-[var(--color-text-primary)] transition-colors text-left"
                                    >
                ...
                <div className="w-full md:w-[320px] bg-[var(--color-chrome)]/30 border-l border-[var(--color-border)] p-6 overflow-y-auto flex flex-col gap-8 shrink-0">
                  <div className="flex flex-col gap-6">
                    ...
                    <button
                      ...
                      className={`group/agent flex items-center gap-3 p-3 rounded-xl border transition-all text-left w-full ${
                        showAgent
                    ...
                    >
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border transition-colors ${showAgent ? 'bg-[var(--color-accent)] border-[var(--color-accent)]' : 'border-[var(--color-border)] bg-surface-100 dark:bg-surface-950 group-hover/agent:border-[var(--color-accent-dim)]'}`}
                      >
                        <Bot
                          size={14}
                          className={`transition-colors ${showAgent ? 'text-surface-950' : 'text-[var(--color-accent-dim)] group-hover/agent:text-[var(--color-accent)]'}`}
                        />
                      </div>
                      <div className="flex flex-col flex-1">
                        <span
                          className={`text-sm font-semibold flex items-center gap-2 ${showAgent ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-primary)]'}`}
                        >
                          AI Agent
                        </span>
                        <span
                          className={`text-[0.6875rem] font-medium transition-colors ${showAgent ? 'text-[var(--color-accent-dim)]' : 'text-[var(--color-text-muted)] group-hover/agent:text-[var(--color-accent-dim)]'}`}
                        >
                    ...
                    <div className="flex flex-col gap-2.5">
                      <span className="font-hud text-[0.625rem] text-[var(--color-accent-dim)] tracking-widest pl-1">
                        Status
                      </span>
                      <button
                        onClick={() => onUpdate(card.id, { completed: !card.completed })}
                        className={`group/check flex items-center gap-3 p-3 rounded-xl border transition-all text-left w-full ${
                          card.completed
                      ...
                      >
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border transition-colors ${card.completed ? 'bg-emerald-500 border-emerald-500' : 'border-[var(--color-border)] bg-surface-100 dark:bg-surface-950 group-hover/check:border-emerald-400'}`}
                        >
                          <Check
                            size={14}
                            className={`transition-opacity ${card.completed ? 'text-white opacity-100' : 'text-emerald-500 opacity-0 group-hover/check:opacity-100'}`}
                          />
                        </div>
                        <div className="flex flex-col">
                          <span
                            className={`text-sm font-semibold ${card.completed ? 'text-emerald-700 dark:text-emerald-400' : 'text-surface-900 dark:text-surface-100'}`}
                          >
                            {card.completed ? 'Completed' : 'Active Task'}
                          </span>
                          {!card.completed && (
                            <span className="text-[0.6875rem] font-medium text-surface-500 group-hover/check:text-emerald-600 dark:group-hover/check:text-emerald-400 transition-colors">
                    ...
                    <div className="flex flex-col gap-2.5">
                      ...
                      {card.dueDate && !card.completed && (
                        <span
                          className={`text-[0.6875rem] font-medium px-2 py-0.5 rounded-full self-start ${getDueDateBadge(card.dueDate).classes}`}
                        >
                      ...
                      {card.recurrenceType && (
                        <div className="flex flex-col gap-2 mt-1 bg-[var(--color-accent-subtle)]/30 rounded-lg p-2.5 border border-[var(--color-border)]">
                          {card.dueDate ? (
                            <p className="text-xs text-[var(--color-text-muted)]">
                              ...
                              <span className="font-semibold text-[var(--color-text-primary)]">
                                {formatNextDate(getNextRecurrenceDate(card.dueDate, card.recurrenceType))}
                              </span>
                            </p>
                          ) : (
                          ...
                          <div className="flex flex-col gap-1.5 mt-1 pt-2 border-t border-[var(--color-border)]">
                            <span className="text-[0.625rem] font-hud tracking-wider text-[var(--color-text-muted)]">
                              End Repeat
                            </span>
                            <HudDatePicker
                    ...
                    <div className="mt-auto pt-6 flex flex-col gap-2">
                      ...
                      <div className="flex justify-between font-data text-[0.6875rem] text-[var(--color-text-muted)]">
                        <span>Created</span>
                        <span className="text-[var(--color-text-secondary)]" title={formatDate(card.createdAt, true)}>
                          {formatRelativeTime(card.createdAt)}
                        </span>
                      </div>
                      <div className="flex justify-between font-data text-[0.6875rem] text-[var(--color-text-muted)]">
                        <span>Updated</span>
                        <span className="text-[var(--color-text-secondary)]" title={formatDate(card.updatedAt, true)}>
                ...
                <div
                  className={`shrink-0 overflow-hidden transition-all duration-300 ease-out border-l ${
                    showAgent ? 'w-[360px] xl:w-[420px] border-[var(--color-border)]' : 'w-0 border-transparent'
                  }`}
                >
