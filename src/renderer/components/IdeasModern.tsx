          ...
          searchQuery || filter !== 'all' ? (
            ...
            <div className="mt-12">
              <EmptyFeatureState
                ...
                benefits={[
                  'Quick capture — never lose an idea',
                  'AI helps assess feasibility',
                  'Convert ideas to project cards',
                ]}
          ...
          <div className="columns-1 md:columns-2 xl:columns-3 gap-6 space-y-6">
            {sortedIdeas.map((idea) => (
              ...
              >
                ...
                {idea.description && <p className="text-sm text-surface-500 line-clamp-3 mb-4">{idea.description}</p>}

                <div className="w-full mt-2 flex flex-wrap items-center gap-2">
                  <span
                    className={`text-[0.625rem] font-bold uppercase tracking-wider px-2 py-1 rounded-md border ${STATUS_COLORS_MODERN[idea.status]}`}
                  >
                    {idea.status}
                  </span>
                  ...
                  {idea.tags.map((tag) => (
                    <span
                      key={tag}
                      className="flex items-center gap-1 text-[0.625rem] font-medium text-[var(--color-accent-dim)] bg-[var(--color-accent-subtle)] border border-[var(--color-border-accent)] px-2 py-1 rounded-md"
                    >
                      <Tag size={10} /> {tag}
                    </span>
                  ))}
