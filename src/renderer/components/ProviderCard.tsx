...
const ProviderCard = memo(function ProviderCard({ provider }: ProviderCardProps) {
  ...
  return (
    <div
      className={`hud-panel clip-corner-cut-sm p-4 transition-colors flex flex-col ${
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div
            className="node-point-sm"
            style={{ backgroundColor: meta.color, boxShadow: `0 0 4px ${meta.color}40` }}
          />
      ...
      {testState?.result && (
        <div
          className={`mb-3 p-2 rounded text-xs font-data flex items-center gap-1.5 ${
            testState.result.success ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
          }`}
        >
      ...
      <div className="flex items-center gap-2 mt-auto pt-3 border-t border-[var(--color-border)]">
        ...
        >
          {testState?.loading ? <Loader2 size={14} className="animate-spin" /> : <Zap size={14} />}
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-xs ml-auto transition-colors font-data ${
            confirmDelete ? 'text-red-400 hover:text-red-300' : 'text-[var(--color-text-muted)] hover:text-red-400'
          }`}
        >
          <Trash2 size={14} />
