...
const BoardColumnModern = memo(function BoardColumnModern({
  ...
  const columnBgStyle = column.color ? { backgroundColor: `${column.color}08` } : undefined;
  const columnBorderStyle = column.color ? { borderColor: `${column.color}30` } : undefined;
  ...
  return (
    ...
    >
      ...
      <div
        ref={headerRef}
        className="group px-4 py-3 flex items-center justify-between cursor-grab active:cursor-grabbing border-b border-transparent hover:border-[var(--color-border)] transition-colors rounded-t-2xl"
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <GripVertical
            size={16}
            className="text-surface-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
          />
                ...
                <div className="absolute right-0 top-full mt-1 bg-white dark:bg-surface-900 border border-[var(--color-border)] rounded-xl shadow-lg py-1.5 z-40 animate-in fade-in zoom-in-95 duration-100 min-w-[120px]">
                  {COLUMN_COLORS.map((c) => (
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        updateColumnColor(column.id, c.value);
                        setShowColorPicker(false);
                      }}
      ...
      <div className="px-3 pb-3 flex flex-col gap-3">
        ...
        {columnCards.length === 0 && !addingCard && (
          <div className="flex flex-col items-center justify-center py-8 text-surface-400 border-2 border-dashed border-surface-200 dark:border-surface-700 rounded-xl">
            ...
            <button onClick={() => setAddingCard(true)} className="mt-2 text-xs text-primary-600 hover:underline">
              Create one
            </button>
          </div>
        )}
      ...
      <div
        className={`px-3 pb-3 pt-1 sticky bottom-0 rounded-b-2xl z-10 ${
          column.color
            ? ''
            : 'bg-gradient-to-t from-surface-50 via-surface-50 to-transparent dark:from-surface-900 dark:via-surface-900'
        }`}
      >
        {addingCard ? (
