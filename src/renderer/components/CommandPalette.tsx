...
function CommandPalette({ isOpen, onClose, navigate, onShowShortcuts }: CommandPaletteProps) {
  ...
  const createCardInFirstColumn = useCallback(
  ...
  const actionItems: CommandItem[] = useMemo(
    () => [
      ...
      },
      {
        ...
        icon: Play,
        category: 'Actions',
        action: () => go('/meetings?action=record'),
      },
  ...
  const dataItems: CommandItem[] = useMemo(() => {
    ...
    for (const i of ideas)
      items.push({
        id: `idea-${i.id}`,
        label: i.title,
        sublabel: i.description ? stripHtml(i.description) : undefined,
        icon: Lightbulb,
        category: 'Ideas',
        action: () => go('/ideas'),
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg hud-panel-accent clip-corner-cut shadow-[0_0_40px_rgba(62,232,228,0.15)] overflow-hidden"
      ...
      >
        ...
        <div ref={listRef} className="max-h-80 overflow-y-auto py-2">
          ...
          {grouped.map((group) => (
            <div key={group.category}>
              <div className="px-4 py-1.5 font-hud text-[0.625rem] tracking-widest uppercase text-[var(--color-accent-dim)]">
                {group.category}
              </div>
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 shrink-0 ${sel ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-muted)]'}`}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm truncate">{item.label}</div>
                      {item.sublabel && (
                        <div className="text-xs text-[var(--color-text-muted)] truncate font-data">{item.sublabel}</div>
                      )}
                    </div>
                  </button>
                );
        ...
        <div className="flex items-center gap-4 px-4 py-2 border-t border-[var(--color-border)] text-[0.6875rem] text-[var(--color-text-muted)] font-data">
          <span>
            <kbd className="px-1 py-0.5 bg-[var(--color-accent-subtle)] rounded border border-[var(--color-border)]">
              Enter
            </kbd>{' '}
            select
          </span>
