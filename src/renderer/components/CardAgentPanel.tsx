...
function describeToolEvent(toolName: string, args?: unknown): string {
  ...
  switch (toolName) {
    case 'getCardDetails':
      return 'Looking up card details';
    case 'searchProjectCards':
      return 'Searching project cards';
    case 'addChecklistItem':
      return `Adding checklist item: "${a.title ?? ''}"`;
    case 'toggleChecklistItem':
      return a.completed ? 'Completing checklist item' : 'Uncompleting checklist item';
    case 'addComment':
      return 'Adding comment';
    case 'updateDescription':
      return 'Updating description';
    case 'createCard':
      return `Creating card: "${a.title ?? ''}"`;
    default:
...
function describeToolCall(call: ToolCallRecord): string {
  ...
  switch (call.name) {
    case 'getCardDetails':
      return 'Looked up card details';
    case 'searchProjectCards':
      return 'Searched project cards';
    case 'addChecklistItem':
      return `Added checklist item: "${a.title ?? ''}"`;
    case 'toggleChecklistItem':
      return a.completed ? 'Completed checklist item' : 'Uncompleted checklist item';
    case 'addComment':
      return 'Added comment';
    case 'updateDescription':
      return 'Updated description';
    case 'createCard':
      return `Created card: "${a.title ?? ''}"`;
    default:
        ...
        <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scroll-smooth">
          ...
          {messages.length === 0 && !streaming && (
            <div className="flex flex-col items-center justify-center py-8 px-2">
              ...
              <p className="text-xs text-[var(--color-text-secondary)] mb-1 text-center">
