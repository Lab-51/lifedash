...
export default function DashboardModern() {
  ...
  useEffect(() => {
    ...
    store.loadPreferences().then(() => {
      ...
      const projectIds = prefs?.analyzedProjectIds?.length ? prefs.analyzedProjectIds : undefined; // undefined = all projects
    ...
    const cleanup = window.electronAPI.onBackgroundAgentNewInsights(() => {
      const prefs = useBackgroundAgentStore.getState().preferences;
      const projectIds = prefs?.analyzedProjectIds?.length ? prefs.analyzedProjectIds : undefined;
  ...
  return (
    <div ref={scrollContainerRef} className="relative h-full overflow-y-auto bg-surface-50/50 dark:bg-surface-950">
      ...
      <div className="relative p-8 pb-12">
        <div className="relative z-10 flex items-end justify-between gap-4">
          <div className="shrink-0 p-2">
            <h1 className="font-hud text-2xl text-[var(--color-accent)] text-glow">{getGreeting()}</h1>
            <p className="font-data text-[var(--color-text-secondary)] mt-2">{formatToday()}</p>
          ...
          <div className="flex gap-3 shrink-0">
            ...
            {[
              {
                ...
                colors:
                  'text-[var(--color-accent)] dark:text-[var(--color-accent)] bg-[var(--color-accent-subtle)] border-[var(--color-border-accent)] hover:bg-[var(--color-accent)] hover:text-white hover:border-transparent',
              },
              {
                icon: Brain,
                label: 'Brain',
                path: '/brainstorm?action=create',
                colors:
                  'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/20 hover:bg-indigo-500 hover:text-white dark:hover:bg-indigo-600 hover:border-transparent',
              },
              {
                icon: Lightbulb,
                label: 'Idea',
                path: '/ideas?action=create',
                colors:
                  'text-[var(--color-warm)] dark:text-[var(--color-warm)] bg-warm-50 dark:bg-warm-500/10 border-warm-200 dark:border-warm-500/20 hover:bg-[var(--color-warm)] hover:text-white hover:border-transparent',
              },
            ...
            <button
              ...
              className={`group flex flex-col items-center justify-center w-20 h-20 rounded-2xl transition-all duration-300 border hover:scale-105 hover:shadow-lg hover:shadow-black/5 dark:hover:shadow-white/5 hover:border-transparent ${
            ...
            >
              <Timer
                size={24}
                className={`mb-2 transition-transform duration-300 group-hover:scale-110 ${focusMode === 'focus' || focusMode === 'break' ? 'animate-[spin_4s_linear_infinite]' : ''}`}
              />
          ...
          <div ref={standupRef} className="mb-6 hud-panel-accent clip-corner-cut-sm p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-surface-900 dark:text-surface-100">Daily Standup</h2>
                {standupProjectId ? (
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={() => handleGenerateStandup(standupProjectId)}
                  disabled={generatingStandup}
                  className="p-1.5 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors"
                  title="Regenerate"
                >
        ...
        <div className="grid grid-cols-12 gap-6 pb-8">
          ...
          <div className="col-span-12 sm:col-span-4 lg:col-span-4">
            <div className="hud-panel-accent clip-corner-cut-sm p-6 flex items-center justify-between group cursor-default hover:border-[var(--color-border-bright)] transition-all duration-300 relative overflow-hidden">
              <div className="flex flex-col gap-1 relative z-10">
                <div className="flex items-center gap-2">
                  <span className="font-hud text-xs tracking-widest text-[var(--color-magenta-dim)]" aria-hidden="true">
                    SYS.MEETINGS
                  </span>
                  <div className="h-px w-8 bg-gradient-to-r from-[var(--color-magenta)] to-transparent opacity-40" />
                </div>
                <p className="font-[var(--font-display)] text-3xl text-[var(--color-magenta)] text-glow-magenta">
                  {meetings.length}
                </p>
              </div>
          ...
          <div className="col-span-12 sm:col-span-4 lg:col-span-4">
            <div className="hud-panel-accent clip-corner-cut-sm p-6 flex items-center justify-between group cursor-default hover:border-[var(--color-border-bright)] transition-all duration-300 relative overflow-hidden">
              <div className="flex flex-col gap-1 relative z-10">
                ...
                <p className="font-[var(--font-display)] text-3xl text-[var(--color-accent)] text-glow">
                  {projects.length}
                </p>
              </div>
          ...
          <div className="col-span-12 sm:col-span-4 lg:col-span-4">
            <div className="hud-panel-accent clip-corner-cut-sm p-6 flex items-center justify-between group cursor-default hover:border-[var(--color-border-bright)] transition-all duration-300 relative overflow-hidden">
              <div className="flex flex-col gap-1 relative z-10">
                <div className="flex items-center gap-2">
                  <span className="font-hud text-xs tracking-widest text-[var(--color-warm-dim)]" aria-hidden="true">
                    SYS.IDEAS
                  </span>
                  <div className="h-px w-8 bg-gradient-to-r from-[var(--color-warm)] to-transparent opacity-40" />
                </div>
                <p className="font-[var(--font-display)] text-3xl text-[var(--color-warm)] text-glow-warm">
                  {ideas.length}
                </p>
              </div>
            ...
            <div className="hud-panel clip-corner-cut-sm overflow-hidden h-full flex flex-col">
              <div className="p-5 flex justify-between items-center shrink-0">
                ...
                <button
                  ...
                  className="text-sm text-[var(--color-magenta)] hover:text-[var(--color-magenta-dim)] font-medium flex items-center gap-1 py-1 px-2 transition-colors"
                >
                ...
                {recentMeetings.length === 0 ? (
                  ...
                  recentMeetings.map((meeting) => (
                    ...
                    >
                      ...
                      <h4 className="font-bold text-[var(--color-text-primary)] mb-1 group-hover:text-[var(--color-magenta)] transition-colors line-clamp-1">
                        {meeting.title}
                      </h4>
            ...
            <div className="hud-panel clip-corner-cut-sm h-full flex flex-col">
                ...
                {activeProjects.length === 0 ? (
                  <EmptyFeatureState
                    ...
                    benefits={[
                      ...
                      'Add details, checklists, and due dates',
                    ]}
                      ...
                      >
                        ...
                        <div className="flex-1 pb-2">
                          <div className="flex items-center gap-2">
                            ...
                            {project.pinned && <Star size={10} className="text-amber-400 fill-amber-400 shrink-0" />}
                          </div>
              ...
              <div className="p-3 shrink-0 mt-auto">
                <button
                  ...
                  className="w-full py-2 flex items-center justify-center gap-1 text-xs font-hud text-[var(--color-text-secondary)] hover:text-[var(--color-accent)] hover:bg-[var(--color-accent-subtle)] rounded-lg transition-colors"
                >
            ...
            <div className="hud-panel clip-corner-cut-sm overflow-hidden">
              <div className="p-5 flex justify-between items-center">
                ...
                {pendingActions.length > 0 && (
                  <button
                    onClick={() => navigate('/meetings')}
                    className="text-sm text-[var(--color-warm)] hover:text-[var(--color-warm-dim)] font-medium flex items-center gap-1 py-1 px-2 transition-colors"
                  >
                    View All <ArrowRight size={14} />
                  </button>
                )}
                ...
                {pendingActions.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-sm text-[var(--color-text-secondary)]">
                      Complete a meeting to see action items here
                    </p>
                  </div>
                ) : (
                    ...
                    {pendingActions.map((item) => (
                      ...
                      >
                        <div className="mt-0.5 shrink-0">
                          <CircleDot
                            size={14}
                            className={item.status === 'approved' ? 'text-emerald-500' : 'text-[var(--color-warm)]'}
                          />
                        </div>
                        ...
                        <span
                          className={`shrink-0 font-data text-[0.625rem] px-2 py-0.5 rounded-full border ${
      ...
      {standupPickerOpen &&
        createPortal(
          <>
            <div style={{ position: 'fixed', inset: 0, zIndex: 99999 }} onClick={() => setStandupPickerOpen(false)} />
            <div
              style={{
                position: 'fixed',
                zIndex: 99999,
