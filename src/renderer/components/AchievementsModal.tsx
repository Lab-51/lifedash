...
import { X, Zap, Trophy, CheckSquare, FolderPlus, Mic, Lightbulb, Brain, LayoutGrid, Lock } from 'lucide-react';
import FocusTrap from './FocusTrap';
import { useGamificationStore } from '../stores/gamificationStore';
import { ACHIEVEMENTS } from '../../shared/types/gamification';
// ICON_MAP now lives in achievementIconMap.ts (leaf module) so AchievementBanner
// can import it without creating a cycle through gamificationStore.
// Re-exported here so any older callers that imported ICON_MAP from this file
// continue to work without a code change. (CODE-Q.1 Task 1.)
...
const CATEGORY_CONFIG: Array<{
  ...
  {
    key: 'cross',
    label: 'Cross-Feature Achievements',
    icon: LayoutGrid,
...
function AchievementsModal({ isOpen, onClose }: AchievementsModalProps) {
  ...
  const unlockedMap = new Map(achievements.filter((a) => a.unlockedAt !== null).map((a) => [a.id, a.unlockedAt!]));
                          ...
                          <div className="flex items-start gap-2.5">
                            ...
                            <div
                              className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
                                unlocked ? `${cat.bgColor} ${cat.textColor}` : 'bg-surface-700/50 text-surface-500'
                              }`}
                            >
