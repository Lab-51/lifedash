// === FILE PURPOSE ===
// Brainstorm page — Classic Design
...
export default function BrainstormClassic() {
                                ...
                                <div className="grid grid-cols-1 gap-1.5">
                                    {BRAINSTORM_TEMPLATES.map((template) => (
                                        <button
                                            ...
                                            className={`flex items-start gap-2 p-2 rounded-lg border text-left transition-colors ${selectedTemplateId === template.id
                                                    ? 'border-primary-500 bg-primary-600/10'
                                                    : 'border-surface-700 bg-surface-800 hover:border-surface-600'
                                                }`}
                                        >
                                            <span className={`mt-0.5 flex-shrink-0 ${selectedTemplateId === template.id ? 'text-primary-400' : 'text-surface-500'
                                                }`}>
                                                {templateIcons[template.icon]}
                                            </span>
                                            <div className="min-w-0">
                                                <p className={`text-xs font-medium ${selectedTemplateId === template.id ? 'text-primary-300' : 'text-surface-300'
                                                    }`}>{template.name}</p>
                        ...
                        {loadingSessions ? (
                            ...
                            filteredSessions.map((session) => (
                                <div
                                    ...
                                    className={`group flex items-center justify-between px-3 py-2.5 cursor-pointer border-l-2 transition-colors ${activeSession?.id === session.id
